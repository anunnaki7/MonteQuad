...
const DUST = [
  { x: 5,  y: 64, size: 5,   duration: 2.6, delay: 0.0, drift: -60 },
  { x: 10, y: 70, size: 3,   duration: 3.0, delay: 0.5, drift: -42 },
  { x: 3,  y: 74, size: 7,   duration: 2.3, delay: 1.0, drift: -75 },
  { x: 14, y: 60, size: 4,   duration: 3.4, delay: 1.4, drift: -48 },
  { x: 7,  y: 78, size: 5.5, duration: 2.7, delay: 1.9, drift: -55 },
  { x: 83, y: 64, size: 5,   duration: 2.6, delay: 0.3, drift: 60 },
  { x: 89, y: 70, size: 3,   duration: 3.0, delay: 0.8, drift: 42 },
  { x: 93, y: 74, size: 7,   duration: 2.3, delay: 1.2, drift: 75 },
  { x: 86, y: 60, size: 4,   duration: 3.4, delay: 1.7, drift: 48 },
  { x: 96, y: 78, size: 5.5, duration: 2.7, delay: 0.1, drift: 55 },
];

const SPEEDLINES = [
  { y: 28, delay: 0.0, opacity: 0.10 },
  { y: 36, delay: 0.4, opacity: 0.07 },
  { y: 44, delay: 0.8, opacity: 0.12 },
  { y: 53, delay: 0.2, opacity: 0.06 },
  { y: 61, delay: 1.1, opacity: 0.09 },
];
...
const RpmGauge = ({ counter, isVisible }: { counter: number; isVisible: boolean }) => {
  ...
  // arc from 215° going clockwise, span = 250°
  const arcSpan = 250;
  ...
  return (
    ...
    >
      ...
      />
      {/* Hub dot */}
      <circle cx={cx} cy={cy} r={4} fill="hsl(168 100% 60%)" style={{ filter: "drop-shadow(0 0 5px hsl(168 100% 55%))" }} />
      {/* RPM label */}
      <text x={cx} y={cy + 16} textAnchor="middle" fontSize="7" fill="hsl(0 0% 100% / 0.25)" fontFamily="'Space Grotesk', sans-serif" letterSpacing="2">RPM</text>
      {/* Gradient def */}
      <defs>
...
const Preloader = ({ onComplete }: { onComplete: () => void }) => {
  ...
  useEffect(() => {
    ...
    const doneTimer = setTimeout(() => onComplete(), 2950);
    ...
    <>
      <style>{`
        ...
        @keyframes exhaustPuff {
          ...
          20%  { opacity: 0.55; }
          100% { opacity: 0;   transform: translateX(-45px) scale(1.6); }
        }
      `}</style>
      ...
      >
        ...
        <div
          style={{
            ...
            height: "38%",
            background: "linear-gradient(0deg, hsl(150 45% 7% / 0.5) 0%, transparent 100%)",
            pointerEvents: "none",
          }}
        />

        {/* ── Tyre tracks at bottom ── */}
        <TyreTrack bottom={30} delay={0} />
        <TyreTrack bottom={8}  delay={0.7} />
        ...
        <div
          style={{
            ...
            height: "min(580px, 125vw)",
            borderRadius: "50%",
            background:
              "radial-gradient(ellipse at center, hsl(150 60% 18% / 0.18) 0%, transparent 68%)",
            filter: "blur(55px)",
        ...
        >
          ...
          <div style={{ position: "relative", display: "inline-flex" }}>
            {[
              { top: -14, left: -14,  borderTop: "2px solid hsl(168 100% 52% / 0.65)", borderLeft:  "2px solid hsl(168 100% 52% / 0.65)", animDelay: "0s" },
              { top: -14, right: -14, borderTop: "2px solid hsl(168 100% 52% / 0.65)", borderRight: "2px solid hsl(168 100% 52% / 0.65)", animDelay: "0.4s" },
              { bottom: -14, left: -14,  borderBottom: "2px solid hsl(168 100% 52% / 0.65)", borderLeft:  "2px solid hsl(168 100% 52% / 0.65)", animDelay: "0.8s" },
              { bottom: -14, right: -14, borderBottom: "2px solid hsl(168 100% 52% / 0.65)", borderRight: "2px solid hsl(168 100% 52% / 0.65)", animDelay: "1.2s" },
            ].map((b, i) => (
              <div
                key={i}
                style={{
                  ...
                  opacity: isVisible ? 1 : 0,
                  transition: `opacity 0.8s ease ${parseFloat(b.animDelay) + 0.5}s`,
                }}
              />
            ))}
          ...
          >
            {/* Exhaust puffs (left of gauge) */}
            <div style={{ position: "relative", width: 28, height: 30, flexShrink: 0 }}>
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  style={{
              ...
              {[0, 1, 2, 3].map((i) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      (enginePulse ? i % 2 === 0 : i % 2 !== 0) && counter > i * 25
                        ? "hsl(168 100% 58%)"
                        : "hsl(150 30% 18%)",
                    boxShadow:
                      (enginePulse ? i % 2 === 0 : i % 2 !== 0) && counter > i * 25
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
              {/* Fuel icon (simple drop shape via CSS) */}
